...
import type { Empresa, Paginated } from "@/lib/types";
import {
  Button,
  Card,
  EmptyState,
...
export default function PrestadoresPage() {
  ...
  async function reload() {
    setLoading(true);
    try {
      const data = await api<Paginated<Empresa>>("/empresas/?page_size=1000");
  ...
  async function remover(c: Empresa) {
    if (!confirm(`Remover o prestador “${c.nome}”?`)) return;
    setMsg(null);
    try {
      await api(`/empresas/${c.id}/`, { method: "DELETE" });
      await reload();
    } catch (e) {
      setMsg(e instanceof ApiError ? e.message : "Erro ao remover o prestador.");
    }
  }
  ...
  const visibleRows = useMemo(() => {
    ...
    const filtrados = q
      ? rows.filter((c) =>
          [c.nome, c.cnpj, c.cidade, c.uf, c.contato_gestor]
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Empresas prestadoras — seus dados saem nos relatórios técnicos."
        actions={
          podeEditar ? (
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            icon={query ? Search : Factory}
            title={query ? "Nada encontrado" : "Nenhum prestador cadastrado"}
            description={
              query
                ? `Nenhum prestador corresponde a “${query}”.`
                : "Cadastre a empresa prestadora para os relatórios."
            }
            action={
              !query && podeEditar ? (
                <Link href="/prestadores/novo">
        ...
        <>
          <Table>
            ...
            <TBody>
              {pageRows.map((c) => (
                <TR key={c.id} onClick={podeEditar ? () => router.push(`/prestadores/${c.id}`) : undefined}>
