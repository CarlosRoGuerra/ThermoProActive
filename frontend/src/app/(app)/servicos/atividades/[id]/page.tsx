...
export default function AtividadePage() {
  ...
  return <div className="space-y-5">
    ...
    {itemId ? (
      !item?.analise ? <Card>Análise não encontrada nesta atividade.</Card> : <Card className="space-y-4">
        ...
        <p className="text-sm text-fg-muted">A análise está vinculada a este equipamento e à rota atual. O formulário técnico de {atividade.tipo_corretiva === "BALANCEAMENTO" ? "balanceamento" : "alinhamento a laser"} estará disponível na próxima fase.</p>
      ...
      <Table><THead><TR><TH>Equipamento</TH><TH>Condição</TH><TH>Estado da análise</TH><TH>Ação</TH></TR></THead>
        <TBody>{atividade.itens.map((i) => <TR key={i.id}>
          ...
          <TD><Badge tone="neutral">{i.analise ? "Análise iniciada" : "Não iniciada"}</Badge></TD>
          <TD><Button disabled={ocupado || (!i.analise && (!podeEditar || !i.condicao))} onClick={() => analisar(i)}>{i.analise ? (podeEditar ? "Continuar análise" : "Visualizar análise") : "Analisar"}</Button></TD>
        </TR>)}</TBody>
      </Table>
    </Card>}
  </div>;
