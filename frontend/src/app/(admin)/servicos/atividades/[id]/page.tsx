...
import { Badge, Button, Card, Field, PageHeader, Select, Spinner, Table, TBody, TD, Textarea, TH, THead, TR } from "@/components/ds";
...
export default function AtividadePage() {
  ...
  useEffect(() => {
    let ativo = true;
    setLoading(true);
    Promise.all([api<AtividadeCorretiva>(`${base}/`), api<Paginated<Condicao>>("/condicoes/?page_size=500")])
  ...
  return <div className="space-y-5">
    <Link href={itemId ? href : "/servicos"} className="text-sm text-primary">{itemId ? "Voltar para equipamentos da rota" : "Voltar para Manutenção corretiva"}</Link>
    ...
    {itemId ? (
      ...
      <Table><THead><TH>Equipamento</TH><TH>Condição</TH><TH>Estado da análise</TH><TH>Ação</TH></THead>
