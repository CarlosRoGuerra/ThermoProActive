...
import { Button, Card, Field, Input, PageHeader, Select } from "@/components/ds";
...
export default function NovaAtividadePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api<Paginated<TecnologiaCorretiva>>("/tecnologias-analise/?page_size=500"),
      clienteAtivo ? api<Paginated<Rota>>(`/rotas/?cliente=${clienteAtivo.id}&page_size=300`) : Promise.resolve({ results: [] }),
    ]).then(([t, r]) => {
  ...
  useEffect(() => {
    ...
    if (tecnologia) api<Paginated<Instrumento>>(`/instrumentos/?tecnologias=${tecnologia}&page_size=500`)
  ...
  return <div className="space-y-5">
    <Link href="/servicos" className="text-sm text-primary">Voltar para Manutenção corretiva</Link>
    <PageHeader title="Nova atividade / Carregar rota" description={clienteAtivo.nome_fantasia || clienteAtivo.nome} />
    <Card>
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Tecnologia" obrigatorio><Select value={tecnologia} disabled={salvando || carregando} onChange={(e) => { setTecnologia(e.target.value); setRota(""); }}>
          <option value="">Selecione a tecnologia</option>
          {tecnologias.map((t) => <option key={t.id} value={t.id}>{t.nome}</option>)}
        </Select></Field>
        <Field label="Rota" obrigatorio><Select value={rota} disabled={!tecnologia || salvando || carregando} onChange={(e) => setRota(e.target.value)}>
          <option value="">Selecione a rota</option>
          {disponiveis.map((r) => <option key={r.id} value={r.id}>{r.nome} ({r.qtd_equipamentos} equipamentos)</option>)}
        </Select></Field>
        <Field label="Instrumento" obrigatorio><Select value={instrumento} disabled={!tecnologia || salvando} onChange={(e) => setInstrumento(e.target.value)}>
        ...
        <Field label="Data de término/ensaio" obrigatorio><Input type="date" value={data} max={hoje()} disabled={salvando} onChange={(e) => setData(e.target.value)} /></Field>
      </div>
