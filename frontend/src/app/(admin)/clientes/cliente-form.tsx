...
export function ClienteForm({ clienteId }: { clienteId?: number }) {
  ...
  if (carregando) {
    return (
      <LoadingState variante="formulario" linhas={4} label="Carregando o cliente…" />
    );
  }
  ...
  return (
    <PageBody>
      <PageHeader
        ...
        description={"Tomador de serviço — é dele que saem as áreas, os setores, os equipamentos e as rotas de inspeção."}
        trilha={[{ label: "Clientes", href: "/clientes" }, { label: editando ? "Editar" : "Novo" }]}
      />
