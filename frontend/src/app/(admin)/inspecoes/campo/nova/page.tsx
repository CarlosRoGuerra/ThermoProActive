...
import { Check, ClipboardList, Save, Search, X } from "lucide-react";
...
export default function CarregarRotaPage() {
  ...
  useEffect(() => {
    ...
    api<Paginated<Rota>>(`/rotas/?cliente=${clienteAtivo.id}&page_size=300`)
  ...
  useEffect(() => {
    ...
    api<Paginated<InstrumentoOpt>>(`/instrumentos/?tecnologias=${tecnologia}&page_size=500`)
  ...
  async function abrirPicker() {
    ...
    try {
      const d = await api<Paginated<Relatorio>>(
        `/relatorios-inspecao/?cliente=${clienteAtivo.id}&tecnologia=${tecnologia}&page_size=300`
      );
  ...
  return (
    <PageBody>
      <PageHeader
        icon={ClipboardList}
        title="Carregar rota"
        description={`Cria a folha de campo com os equipamentos da rota, pronta para registrar as condições em ${clienteAtivo.nome_fantasia || clienteAtivo.nome}.`}
        trilha={[
      ...
      <Card>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Tecnologia" obrigatorio>
        ...
        <div className="mt-4 rounded-lg border border-border bg-surface-muted/30 p-4">
          ...
          {modoNumero === "novo" ? (
            ...
            <div className="mt-3">
              {relatorioSel ? (
                <div className="flex flex-wrap items-center gap-3 rounded-lg border border-primary/40 bg-primary-subtle px-3 py-2">
                ...
                {relatorios.map((r) => (
                  <li key={r.id}>
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-lg border border-border px-3 py-2 text-left transition-colors hover:border-primary hover:bg-primary-subtle"
                    >
