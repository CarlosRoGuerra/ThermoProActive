...
import { AlertTriangle, Calculator, Save, Wrench } from "lucide-react";
...
export function OspForm({ ospId }: { ospId?: number }) {
  ...
  // Portal (Master do cliente): não escolhe empresa — é sempre a própria.
  // `ClienteAtivoProvider` já resolve isso sozinho para quem está logado como
  // cliente, então aqui só falta travar o campo em vez de pedir escolha.
  useEffect(() => {
    if (!editando && ehCliente && clienteAtivo) setCliente(clienteAtivo.id);
  }, [editando, ehCliente, clienteAtivo]);
  ...
  useEffect(() => {
    ...
    api<Paginated<Equipamento>>(`/equipamentos/?setor__area__cliente=${cliente}&page_size=300`)
  ...
  if (carregando) {
    return (
      <LoadingState variante="formulario" linhas={4} label="Carregando a ordem de serviço…" />
    );
  }
  ...
  return (
    <PageBody>
      <PageHeader
        ...
        description={"A ordem registra a intervenção recomendada, a urgência e o prazo. Normalmente ela é gerada pela análise final — este formulário é para os casos manuais."}
        trilha={[{ label: "Ordens de serviço", href: "/osps" }, { label: editando ? "Editar" : "Novo" }]}
      />
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {ehCliente ? (
            ...
            <Field label="Cliente" obrigatorio>
              <Combobox
                value={cliente}
                onChange={(v) => {
                  setCliente(v);
          ...
          <Field label="Equipamento" obrigatorio>
            <Combobox
              value={equipamento}
